...
{
    private static readonly Error AuthenticationFailed = new(Code: "Keycloak.AuthenticationFailed", Name: "Failed to acquire access token do to authentication failure");
    ...
    public async Task<Result<string>> GetAccessTokenAsync(string email, string password, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            HttpResponseMessage response = await httpClient.PostAsync(requestUri: "", authorizationRequestContent, cancellationToken);
            ...
            return authorizationToken?.AccessToken ?? Result.Failure<string>(error: AuthenticationFailed);
        }
        catch (HttpRequestException)
        {
            return Result.Failure<string>(error: AuthenticationFailed);
        }
    }
}