                                               ...
                                               : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        foreach (OutboxMessageResponse outboxMessage in outboxMessages)
        {
            ...
            {
                logger.LogError(caughtException, message: "Exception while processing outbox message {MessageId}", outboxMessage.Id);
    ...
    private async Task UpdateOutboxMessageAsync(IDbConnection connection, IDbTransaction transaction, OutboxMessageResponse outboxMessage, Exception? exception)
    {
        ...
        await connection.ExecuteAsync(sql, param: new { outboxMessage.Id, ProcessedOnUtc = dateTimeProvider.UtcNow, Error = exception?.ToString() }, transaction: transaction);
    }
